    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in StoryArea) in the input: " + _key)

        _obj = cls.model_validate({
            "position": StoryAreaPosition.from_dict(obj["position"]) if obj.get("position") is not None else None,
            "type": StoryAreaType.from_dict(obj["type"]) if obj.get("type") is not None else None
        })
        return _obj
