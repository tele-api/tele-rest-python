...
class PostRestrictChatMemberRequest(BaseModel):
    """
    ...
    until_date: Optional[StrictInt] = Field(default=None, description="Date when restrictions will be lifted for the user; Unix time. If user is restricted for more than 366 days or less than 30 seconds from the current time, they are considered to be restricted forever")
    __properties: ClassVar[List[str]] = ["chat_id", "user_id", "permissions", "use_independent_chat_permissions", "until_date"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in PostRestrictChatMemberRequest) in the input: " + _key)
