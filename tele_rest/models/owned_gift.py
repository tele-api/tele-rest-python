...
OWNEDGIFT_ONE_OF_SCHEMAS = ["OwnedGiftRegular", "OwnedGiftUnique"]
...
class OwnedGift(BaseModel):
    ...
    oneof_schema_1_validator: Optional[OwnedGiftRegular] = None
    # data type: OwnedGiftUnique
    oneof_schema_2_validator: Optional[OwnedGiftUnique] = None
    actual_instance: Optional[Union[OwnedGiftRegular, OwnedGiftUnique]] = None
    one_of_schemas: Set[str] = { "OwnedGiftRegular", "OwnedGiftUnique" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in OwnedGift with oneOf schemas: OwnedGiftRegular, OwnedGiftUnique. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in OwnedGift with oneOf schemas: OwnedGiftRegular, OwnedGiftUnique. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into OwnedGiftRegular
        try:
            instance.actual_instance = OwnedGiftRegular.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into OwnedGiftUnique
        try:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into OwnedGift with oneOf schemas: OwnedGiftRegular, OwnedGiftUnique. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into OwnedGift with oneOf schemas: OwnedGiftRegular, OwnedGiftUnique. Details: " + ", ".join(error_messages))
        else:
            return instance
