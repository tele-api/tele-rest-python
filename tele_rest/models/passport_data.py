...
class PassportData(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["data", "credentials"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in PassportData) in the input: " + _key)

        _obj = cls.model_validate({
            "data": [EncryptedPassportElement.from_dict(_item) for _item in obj["data"]] if obj.get("data") is not None else None,
            "credentials": EncryptedCredentials.from_dict(obj["credentials"]) if obj.get("credentials") is not None else None
        })
        return _obj
