...
class ForceReply(BaseModel):
    ...
    input_field_placeholder: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=64)]] = Field(default=None, description="*Optional*. The placeholder to be shown in the input field when the reply is active; 1-64 characters")
    ...
    __properties: ClassVar[List[str]] = ["force_reply", "input_field_placeholder", "selective"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in ForceReply) in the input: " + _key)

        _obj = cls.model_validate({
            "force_reply": obj.get("force_reply") if obj.get("force_reply") is not None else True,
