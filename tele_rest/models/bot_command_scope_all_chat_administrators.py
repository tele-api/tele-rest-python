...
class BotCommandScopeAllChatAdministrators(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="Scope type, must be *all\\_chat\\_administrators*")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in BotCommandScopeAllChatAdministrators) in the input: " + _key)

        _obj = cls.model_validate({
            "type": obj.get("type") if obj.get("type") is not None else 'all_chat_administrators'
        })
        return _obj
