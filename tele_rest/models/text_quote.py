    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in TextQuote) in the input: " + _key)

        _obj = cls.model_validate({
            "text": obj.get("text"),
            "entities": [MessageEntity.from_dict(_item) for _item in obj["entities"]] if obj.get("entities") is not None else None,
            "position": obj.get("position"),
            "is_manual": obj.get("is_manual") if obj.get("is_manual") is not None else True
        })
        return _obj
