...
class ChatBoostUpdated(BaseModel):
    ...
    chat: Chat
    boost: ChatBoost
    __properties: ClassVar[List[str]] = ["chat", "boost"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in ChatBoostUpdated) in the input: " + _key)

        _obj = cls.model_validate({
            "chat": Chat.from_dict(obj["chat"]) if obj.get("chat") is not None else None,
            "boost": ChatBoost.from_dict(obj["boost"]) if obj.get("boost") is not None else None
        })
        return _obj
