...
class GetChatGiftsRequest(BaseModel):
    """
    Request parameters for getChatGifts
    """ # noqa: E501
    chat_id: SendMessageRequestChatId
    exclude_unsaved: Optional[StrictBool] = Field(default=None, description="Pass *True* to exclude gifts that aren't saved to the chat's profile page. Always *True*, unless the bot has the *can\\_post\\_messages* administrator right in the channel.")
    exclude_saved: Optional[StrictBool] = Field(default=None, description="Pass *True* to exclude gifts that are saved to the chat's profile page. Always *False*, unless the bot has the *can\\_post\\_messages* administrator right in the channel.")
    ...
    offset: Optional[StrictStr] = Field(default=None, description="Offset of the first entry to return as received from the previous request; use an empty string to get the first chunk of results")
    limit: Optional[StrictInt] = Field(default=100, description="The maximum number of gifts to be returned; 1-100. Defaults to 100")
    __properties: ClassVar[List[str]] = ["chat_id", "exclude_unsaved", "exclude_saved", "exclude_unlimited", "exclude_limited_upgradable", "exclude_limited_non_upgradable", "exclude_from_blockchain", "exclude_unique", "sort_by_price", "offset", "limit"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in GetChatGiftsRequest) in the input: " + _key)
