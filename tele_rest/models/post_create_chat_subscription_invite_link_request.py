...
class PostCreateChatSubscriptionInviteLinkRequest(BaseModel):
    """
    ...
    subscription_price: StrictInt = Field(description="The amount of Telegram Stars a user must pay initially and after each subsequent subscription period to be a member of the chat; 1-10000")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in PostCreateChatSubscriptionInviteLinkRequest) in the input: " + _key)

        _obj = cls.model_validate({
            "chat_id": PostCreateChatSubscriptionInviteLinkRequestChatId.from_dict(obj["chat_id"]) if obj.get("chat_id") is not None else None,
            "name": obj.get("name"),
