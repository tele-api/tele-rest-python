...
class UsersShared(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["request_id", "users"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in UsersShared) in the input: " + _key)

        _obj = cls.model_validate({
            "request_id": obj.get("request_id"),
            "users": [SharedUser.from_dict(_item) for _item in obj["users"]] if obj.get("users") is not None else None
        })
        return _obj
