...
class ChatMemberOwner(BaseModel):
    ...
    """ # noqa: E501
    status: StrictStr = Field(description="The member's status in the chat, always “creator”")
    user: User
    ...
    __properties: ClassVar[List[str]] = ["status", "user", "is_anonymous", "custom_title"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in ChatMemberOwner) in the input: " + _key)

        _obj = cls.model_validate({
            "status": obj.get("status") if obj.get("status") is not None else 'creator',
