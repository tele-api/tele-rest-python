...
class MenuButtonWebApp(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="Type of the button, must be *web\\_app*")
    text: StrictStr = Field(description="Text on the button")
    web_app: WebAppInfo
    __properties: ClassVar[List[str]] = ["type", "text", "web_app"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in MenuButtonWebApp) in the input: " + _key)

        _obj = cls.model_validate({
            "type": obj.get("type") if obj.get("type") is not None else 'web_app',
            "text": obj.get("text"),
