...
class PhotoSize(BaseModel):
    ...
    width: StrictInt = Field(description="Photo width")
    height: StrictInt = Field(description="Photo height")
    file_size: Optional[StrictInt] = Field(default=None, description="*Optional*. File size in bytes")
    __properties: ClassVar[List[str]] = ["file_id", "file_unique_id", "width", "height", "file_size"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in PhotoSize) in the input: " + _key)
