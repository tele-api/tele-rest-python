...
class InlineQueryResultPhoto(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="Type of the result, must be *photo*")
    id: StrictStr = Field(description="Unique identifier for this result, 1-64 bytes")
    photo_url: StrictStr = Field(description="A valid URL of the photo. Photo must be in **JPEG** format. Photo size must not exceed 5MB")
    thumbnail_url: StrictStr = Field(description="URL of the thumbnail for the photo")
    photo_width: Optional[StrictInt] = Field(default=None, description="*Optional*. Width of the photo")
    photo_height: Optional[StrictInt] = Field(default=None, description="*Optional*. Height of the photo")
    ...
    caption: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=1024)]] = Field(default=None, description="*Optional*. Caption of the photo to be sent, 0-1024 characters after entities parsing")
    parse_mode: Optional[StrictStr] = Field(default=None, description="*Optional*. Mode for parsing entities in the photo caption. See [formatting options](https://core.telegram.org/bots/api/#formatting-options) for more details.")
    ...
    __properties: ClassVar[List[str]] = ["type", "id", "photo_url", "thumbnail_url", "photo_width", "photo_height", "title", "description", "caption", "parse_mode", "caption_entities", "show_caption_above_media", "reply_markup", "input_message_content"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in InlineQueryResultPhoto) in the input: " + _key)

        _obj = cls.model_validate({
            "type": obj.get("type") if obj.get("type") is not None else 'photo',
            "id": obj.get("id"),
