...
class InlineQueryResultCachedVoice(BaseModel):
    ...
    voice_file_id: StrictStr = Field(description="A valid file identifier for the voice message")
    title: StrictStr = Field(description="Voice message title")
    ...
    __properties: ClassVar[List[str]] = ["type", "id", "voice_file_id", "title", "caption", "parse_mode", "caption_entities", "reply_markup", "input_message_content"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in InlineQueryResultCachedVoice) in the input: " + _key)

        _obj = cls.model_validate({
            "type": obj.get("type") if obj.get("type") is not None else 'voice',
            "id": obj.get("id"),
            "voice_file_id": obj.get("voice_file_id"),
            "title": obj.get("title"),
