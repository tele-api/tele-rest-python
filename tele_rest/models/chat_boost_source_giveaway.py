...
class ChatBoostSourceGiveaway(BaseModel):
    ...
    """ # noqa: E501
    source: StrictStr = Field(description="Source of the boost, always “giveaway”")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in ChatBoostSourceGiveaway) in the input: " + _key)

        _obj = cls.model_validate({
            "source": obj.get("source") if obj.get("source") is not None else 'giveaway',
            "giveaway_message_id": obj.get("giveaway_message_id"),
            "user": User.from_dict(obj["user"]) if obj.get("user") is not None else None,
            "prize_star_count": obj.get("prize_star_count"),
            "is_unclaimed": obj.get("is_unclaimed") if obj.get("is_unclaimed") is not None else True
        })
        return _obj
