...
class SendGiftRequest(BaseModel):
    ...
    gift_id: StrictStr = Field(description="Identifier of the gift; limited gifts can't be sent to channel chats")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in SendGiftRequest) in the input: " + _key)

        _obj = cls.model_validate({
            "user_id": obj.get("user_id"),
            "chat_id": SendGiftRequestChatId.from_dict(obj["chat_id"]) if obj.get("chat_id") is not None else None,
