...
class InlineQueryResultCachedDocument(BaseModel):
    ...
    document_file_id: StrictStr = Field(description="A valid file identifier for the file")
    ...
    __properties: ClassVar[List[str]] = ["type", "id", "title", "document_file_id", "description", "caption", "parse_mode", "caption_entities", "reply_markup", "input_message_content"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in InlineQueryResultCachedDocument) in the input: " + _key)

        _obj = cls.model_validate({
            "type": obj.get("type") if obj.get("type") is not None else 'document',
            "id": obj.get("id"),
            "title": obj.get("title"),
            "document_file_id": obj.get("document_file_id"),
