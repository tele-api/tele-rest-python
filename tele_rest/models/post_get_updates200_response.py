...
class PostGetUpdates200Response(BaseModel):
    """
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in PostGetUpdates200Response) in the input: " + _key)

        _obj = cls.model_validate({
            "ok": obj.get("ok") if obj.get("ok") is not None else True,
            "result": [Update.from_dict(_item) for _item in obj["result"]] if obj.get("result") is not None else None
        })
        return _obj
