...
class InputMediaVideo(BaseModel):
    ...
    has_spoiler: Optional[StrictBool] = Field(default=None, description="*Optional*. Pass *True* if the video needs to be covered with a spoiler animation")
    __properties: ClassVar[List[str]] = ["type", "media", "thumbnail", "cover", "start_timestamp", "caption", "parse_mode", "caption_entities", "show_caption_above_media", "width", "height", "duration", "supports_streaming", "has_spoiler"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in InputMediaVideo) in the input: " + _key)
