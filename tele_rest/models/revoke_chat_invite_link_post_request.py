...
class RevokeChatInviteLinkPostRequest(BaseModel):
    """
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in RevokeChatInviteLinkPostRequest) in the input: " + _key)

        _obj = cls.model_validate({
            "chat_id": RevokeChatInviteLinkPostRequestChatId.from_dict(obj["chat_id"]) if obj.get("chat_id") is not None else None,
