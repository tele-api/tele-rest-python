...
class InaccessibleMessage(BaseModel):
    ...
    var_date: StrictInt = Field(description="Always 0. The field can be used to differentiate regular and inaccessible messages.", alias="date")
    __properties: ClassVar[List[str]] = ["chat", "message_id", "date"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in InaccessibleMessage) in the input: " + _key)
