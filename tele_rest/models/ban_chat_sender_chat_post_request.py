...
class BanChatSenderChatPostRequest(BaseModel):
    """
    BanChatSenderChatPostRequest
    """ # noqa: E501
    chat_id: SendMessagePostRequestChatId
    sender_chat_id: StrictInt = Field(description="Unique identifier of the target sender chat")
    __properties: ClassVar[List[str]] = ["chat_id", "sender_chat_id"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in BanChatSenderChatPostRequest) in the input: " + _key)

        _obj = cls.model_validate({
            "chat_id": SendMessagePostRequestChatId.from_dict(obj["chat_id"]) if obj.get("chat_id") is not None else None,
            "sender_chat_id": obj.get("sender_chat_id")
        })
        return _obj
