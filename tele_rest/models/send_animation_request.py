...
class SendAnimationRequest(BaseModel):
    ...
    duration: Optional[StrictInt] = Field(default=None, description="Duration of sent animation in seconds")
    width: Optional[StrictInt] = Field(default=None, description="Animation width")
    height: Optional[StrictInt] = Field(default=None, description="Animation height")
    thumbnail: Optional[StrictStr] = None
    caption: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=1024)]] = Field(default=None, description="Animation caption (may also be used when resending animation by *file\\_id*), 0-1024 characters after entities parsing")
    parse_mode: Optional[StrictStr] = Field(default=None, description="Mode for parsing entities in the animation caption. See [formatting options](https://core.telegram.org/bots/api/#formatting-options) for more details.")
    ...
    has_spoiler: Optional[StrictBool] = Field(default=None, description="Pass *True* if the animation needs to be covered with a spoiler animation")
    ...
    __properties: ClassVar[List[str]] = ["business_connection_id", "chat_id", "message_thread_id", "animation", "duration", "width", "height", "thumbnail", "caption", "parse_mode", "caption_entities", "show_caption_above_media", "has_spoiler", "disable_notification", "protect_content", "allow_paid_broadcast", "message_effect_id", "reply_parameters", "reply_markup"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if animation (nullable) is None
        # and model_fields_set contains the field
        if self.animation is None and "animation" in self.model_fields_set:
            _dict['animation'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in SendAnimationRequest) in the input: " + _key)
