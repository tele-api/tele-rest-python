...
class Gifts(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["gifts"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in Gifts) in the input: " + _key)

        _obj = cls.model_validate({
            "gifts": [Gift.from_dict(_item) for _item in obj["gifts"]] if obj.get("gifts") is not None else None
        })
        return _obj
