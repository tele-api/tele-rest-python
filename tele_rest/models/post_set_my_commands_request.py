...
class PostSetMyCommandsRequest(BaseModel):
    """
    ...
    language_code: Optional[StrictStr] = Field(default=None, description="A two-letter ISO 639-1 language code. If empty, commands will be applied to all users from the given scope, for whose language there are no dedicated commands")
    __properties: ClassVar[List[str]] = ["commands", "scope", "language_code"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in PostSetMyCommandsRequest) in the input: " + _key)

        _obj = cls.model_validate({
            "commands": [BotCommand.from_dict(_item) for _item in obj["commands"]] if obj.get("commands") is not None else None,
            "scope": BotCommandScope.from_dict(obj["scope"]) if obj.get("scope") is not None else None,
