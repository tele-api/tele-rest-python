...
class MessageOriginChat(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="Type of the message origin, always “chat”")
    var_date: StrictInt = Field(description="Date the message was sent originally in Unix time", alias="date")
    sender_chat: Chat
    author_signature: Optional[StrictStr] = Field(default=None, description="*Optional*. For messages originally sent by an anonymous chat administrator, original message author signature")
    __properties: ClassVar[List[str]] = ["type", "date", "sender_chat", "author_signature"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in MessageOriginChat) in the input: " + _key)
